...
export function app(): express.Express {
  ...
  const distFolder = join(process.cwd(), 'dist/insurance-agent/browser');
  ...
  const template = fs
    .readFileSync(
      path.join(join(process.cwd(), 'dist/insurance-agent/browser'), 'index.html')
    )
    .toString();
    ...
    res.render(
      ...
      async (err, html) => {
        ...
        if (
          ...
          const talent = {
            name: post?.title || post?.albumname || 'InsuranceAgent.tube Post',
